...
from src.DWM.engine_concat import *
...
patience = 10
convert = 'L'
model_path = rf'runs\watermark\lena_256_gray'
resume = False

####WATERMARK #################
img_path = rf'datasets\watermark_img\lena\lena.jpg'
wm_size = imgsize
convert_wm = 'L'
wm_channel = 1
round_wm = False
...
if resume:
    ...
    save_result = os.path.join(runs,'watermark_cocat',result_path)
    temp = save_result
    i= 1
    while True:
...
def main():
    ...
    model_H = UnetGenerator(input_nc=1+wm_channel,output_nc=1).to(device)
    ...
    if use_pretrained:
        ...
        model_E,optimizer_E = use_weights(model_path+'/E/best.pt',
                                        model_E,
