...
def use_E_weights(checkpoint_fpath, model, optimizer, freez_layer=False):
...
def use_H_weights(checkpoint_fpath, model, optimizer):
    ...
    nlayer = len(layer_names)//2
    trainable_layers = [ layer for i,layer in enumerate(layer_names) if  i>nlayer]
